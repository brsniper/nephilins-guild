...
const hashPassword = (password: string): string => {
  // Simple hash function - same password always produces same hash
  let hash = 0;
...
const initializeAuth = () => {
  // Credenciais são verificadas diretamente no handleLogin
...
const App: React.FC = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState<boolean>(() => {
    return localStorage.getItem('nephilins_logged_in') === 'true';
  });
  const [currentUser, setCurrentUser] = useState<string>(() => {
    return localStorage.getItem('nephilins_current_user') || '';
  });
  ...
  const handleLogin = () => {
    ...
    // Credenciais padrão
    if (loginData.username === 'Brshrek' && loginData.password === 'Jesus321*') {
      setIsLoggedIn(true);
      setCurrentUser(loginData.username);
      localStorage.setItem('nephilins_logged_in', 'true');
      localStorage.setItem('nephilins_current_user', loginData.username);
  ...
  const handleLogout = () => {
    setIsLoggedIn(false);
    setCurrentUser('');
    localStorage.setItem('nephilins_logged_in', 'false');
    localStorage.setItem('nephilins_current_user', '');
    setLoginData({ username: '', password: '' });
  };
  ...
  const handleExportData = () => {
    ...
    link.href = url;
    link.download = `nephilins_backup_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const handleImportData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!confirm('Esta ação irá substituir todos os dados atuais pelos dados do arquivo. Deseja continuar?')) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (json.pts) setPts(json.pts);
        if (json.classes) setClasses(json.classes);
        if (json.rewards) setRewards(json.rewards);
        if (json.presence) setPresence(json.presence);
        if (json.guildLogo) setGuildLogo(json.guildLogo);
        alert('Dados importados com sucesso!');
      } catch (err) {
        alert('Erro ao importar arquivo. Certifique-se de que é um JSON de backup válido.');
      }
    };
  ...
  const generateModernLogo = async () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para gerar logo');
      return;
    }
  ...
  const handleAddPT = () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para adicionar PT');
      return;
    }
  ...
  const handleAddClass = () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para adicionar classe');
      return;
    }
  ...
  const removeClass = (cls: string) => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para remover classe');
      return;
    }
  ...
  const submitNewPlayer = () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para adicionar jogador');
      return;
    }
  ...
  const submitGlobalRecruit = () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para recrutar');
      return;
    }
  ...
  const submitNewReward = () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para adicionar prêmio');
      return;
    }
  ...
  const submitNewSession = () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para registrar batalha');
      return;
    }
    setPresence(prev => [{
      id: crypto.randomUUID(),
  ...
  const resetData = () => {
    if (!isLoggedIn) {
      alert('Você precisa estar logado para resetar dados');
      return;
    }
      ...
      <nav className="sticky top-0 z-40 bg-slate-900/90 backdrop-blur-xl border-b border-sky-900/30 px-4 py-3 mb-6 shadow-2xl">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 justify-between items-center">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <div className="flex bg-slate-800/50 rounded-xl border border-slate-800 overflow-hidden">
              <button onClick={handleExportData} title="Exportar Backup (Salvar no PC)" className="p-2.5 text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 transition-all border-r border-slate-800"><FileDown size={20} /></button>
              <button onClick={() => fileInputRef.current?.click()} disabled={!isLoggedIn} title={isLoggedIn ? "Importar Backup (Carregar do PC)" : "Faça login para importar"} className="p-2.5 text-slate-400 hover:text-sky-400 hover:bg-sky-500/10 transition-all border-r border-slate-800 disabled:opacity-50"><FileUp size={20} /></button>
              <button onClick={() => setIsManagingClasses(true)} disabled={!isLoggedIn} title={isLoggedIn ? "Arsenal de Classes" : "Faça login para gerenciar"} className="p-2.5 text-slate-400 hover:text-sky-400 hover:bg-sky-500/10 transition-all border-r border-slate-800 disabled:opacity-50"><BookOpen size={20} /></button>
              <button onClick={resetData} disabled={!isLoggedIn} title={isLoggedIn ? "Resetar Banco de Dados" : "Faça login para resetar"} className="p-2.5 text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-all disabled:opacity-50"><Trash2 size={20} /></button>
            </div>
            ...
            {isLoggedIn ? (
              <button 
                ...
                className="flex items-center gap-2 bg-red-600 hover:bg-red-500 text-white px-3 py-2 rounded-xl text-xs font-bold transition-all border border-red-500/30"
                title={`Logado como ${currentUser}`}
              >
                <LogOut size={16} /> Sair
              </button>
            ) : (
              <button 
                ...
                className="flex items-center gap-2 bg-sky-600 hover:bg-sky-500 text-white px-3 py-2 rounded-xl text-xs font-bold transition-all border border-sky-500/30 shadow-lg shadow-sky-900/20"
              >
                <LogIn size={16} /> Login
              </button>
            )}
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  disabled={!isLoggedIn}
                  className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white px-4 py-2.5 rounded-xl text-xs font-black uppercase transition-all shadow-lg shadow-emerald-900/20"
                >
                ...
                <button 
                  ...
                  disabled={!isLoggedIn}
                  className="flex items-center gap-2 bg-sky-600 hover:bg-sky-500 disabled:opacity-50 text-white px-4 py-2.5 rounded-xl text-xs font-black uppercase transition-all shadow-lg shadow-sky-900/20"
                >
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 ${selectedPTId ? 'lg:grid-cols-1 xl:grid-cols-1 max-w-2xl mx-auto' : 'lg:grid-cols-3 xl:grid-cols-4'} gap-6`}>
              {filteredPts.map(pt => (
                <PTCard 
                  ...
                  onAddPlayer={(id) => isLoggedIn ? setPlayerModal({ isOpen: true, ptId: id }) : alert('Você precisa estar logado')}
                  onDeletePT={(id) => isLoggedIn && confirm('Apagar PT?') && setPts(pts.filter(p=>p.id!==id))}
                  onRemovePlayer={(ptId, pId) => {
                    if (!isLoggedIn) { alert('Você precisa estar logado'); return; }
          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-8">
              ...
              <button 
                ...
                disabled={!isLoggedIn}
                className="bg-yellow-500 hover:bg-yellow-400 disabled:opacity-50 text-slate-950 px-6 py-3 rounded-2xl font-black text-sm transition-all shadow-xl shadow-yellow-500/20 flex items-center gap-2"
              >
          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-8">
              ...
              <button 
                ...
                disabled={!isLoggedIn}
                className="bg-sky-600 hover:bg-sky-500 disabled:opacity-50 text-white px-6 py-3 rounded-2xl font-black text-sm transition-all shadow-xl shadow-sky-500/20 flex items-center gap-2 border border-sky-400/20"
              >
                ...
                <div key={session.id} className="bg-slate-900/40 border border-slate-800 rounded-3xl overflow-hidden">
                  ...
                  <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    {allPlayerNames.map(name => {
                      ...
                      return (
                        <button key={name} onClick={() => toggleAttendance(session.id, name)} disabled={!isLoggedIn} className={`flex items-center gap-2 p-2.5 rounded-xl border text-left transition-all group disabled:opacity-50 ${isPresent ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-slate-950/50 border-slate-800 text-slate-500 hover:border-slate-600'}`}>
          ...
          <div className="bg-slate-900 border border-sky-900/30 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-2xl font-black text-white">Acesso Restrito</h3>
              </div>
              <p className="text-slate-400 text-sm mb-6">Faça login para gerenciar a guild</p>
              
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-sky-500 uppercase block mb-2">Usuário</label>
                  <input 
                    type="text" 
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 ring-sky-500 text-white"
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-sky-500 uppercase block mb-2">Senha</label>
                  <input 
                    type="password" 
            ...
            <div className="p-6 bg-slate-950 flex gap-3 border-t border-slate-800">
              <button onClick={() => setIsLoginModalOpen(false)} className="flex-1 text-slate-500 font-bold text-xs uppercase hover:text-white transition-colors">Cancelar</button>
              <button onClick={handleLogin} className="flex-1 bg-sky-600 hover:bg-sky-500 text-white py-3 rounded-xl font-bold text-xs uppercase transition-all">Entrar</button>
            </div>
          </div>
        </div>
      )}
